...
def normalize_rows(nodes, n_rows, row_id, method='l1'):
    ...
    """
    nodes = unsorted_segment_normalization(nodes, segment_ids=row_id, num_segments=n_rows, method=method)
    return nodes
...
def normalize_rows_for_graph(graph, method='l1'):
    ...
    `method`: set `unsorted_segment_normalization`
    """
    nodes = normalize_rows(tf.reshape(graph.nodes, [-1]), n_rows=tf.reduce_sum(graph.n_row), row_id=graph.row_id, method=method)
